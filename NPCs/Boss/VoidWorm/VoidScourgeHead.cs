...
namespace VengenceMod.NPCs.Boss.VoidWorm
{
    [AutoloadBossHead]
    public class VoidScourgeHead : ModNPC
    {

        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Scourge of the void");
            Main.npcFrameCount[npc.type] = 1;
        }
        public override void SetDefaults()
        {
            npc.lifeMax = 54000;        //this is the npc health
            ...
            music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/ScourgeOfTheVoid");
        }
        public override void BossLoot(ref string name, ref int potionType)
        {
            potionType = ItemID.GreaterHealingPotion;
            if (Main.rand.Next(6) == 0)
                Item.NewItem(npc.getRect(), ModContent.ItemType<SpiritRaven>(), 1);
            if (Main.rand.Next(6) == 0)
                Item.NewItem(npc.getRect(), ModContent.ItemType<GambitOfTheVoid>(), 1);
            if (Main.rand.Next(6) == 0)
                Item.NewItem(npc.getRect(), ModContent.ItemType<DarkPhantom>(), 1);
            if (Main.rand.Next(6) == 0)
                Item.NewItem(npc.getRect(), ModContent.ItemType<MOTV>(), 1);
            Item.NewItem(npc.getRect(), ItemType<VoidCells>(), 10);
            ...
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("VoidScourgeBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("VoidScourgeTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
